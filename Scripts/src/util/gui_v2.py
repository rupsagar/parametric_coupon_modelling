...
class gui():
    ...
    def selectCoupon(self, event):
        ...
        ## create tabs
        self.tabGroup = ttk.Notebook(self.paramFrame)
        self.tabs = 4*[None]
        self.tabNames = ['Geometry','Mesh','Material','Step']
        for i in range(len(self.tabs)):
            self.tabs[i] = ttk.Frame(self.tabGroup) 
            self.tabGroup.add(self.tabs[i], text=self.tabNames[i])  
        self.tabGroup.place(relx=0.01, rely=0.02, relwidth=0.485, relheight=0.81)
        self.populateParams22(self.couponParams, self.couponGUIParams)
    ...
    def imageSlider(self):
        ...
        self.imgFrameWidth, self.imgFrameHeight = 0.485*self.paramFrameWidth, 0.75*self.paramFrameHeight
        self.imgFrame = ttk.LabelFrame(self.paramFrame, text='Coupon figure')
        self.imgFrame.place(relx=0.505, rely=0.05, width=self.imgFrameWidth, height=self.imgFrameHeight)
        ...
        while True:
            ...
            if imgAspectRatio>=frameAspectRatio:
                openImgFile = openImgFile.resize((int(imgLabelRelWidth*self.imgFrameWidth), int(int(imgLabelRelWidth*self.imgFrameWidth)/imgAspectRatio)), Image.Resampling.LANCZOS)
            elif imgAspectRatio<frameAspectRatio:
                openImgFile = openImgFile.resize((int(int(imgLabelRelHeight*self.imgFrameHeight)*imgAspectRatio), int(imgLabelRelHeight*self.imgFrameHeight)), Image.Resampling.LANCZOS)
        ...
        ## left button
        self.leftButton = tk.Button(self.imgFrame, text="<", fg='red', font=('', 11, 'bold'), command=left)
        self.leftButton.place(relx=0.05, rely=0.45, relwidth=0.05, relheight=0.05)
        ## right button
        self.rightButton = tk.Button(self.imgFrame, text=">", fg='red', font=('', 11, 'bold'), command=right)
    ...
    def totalNumKeys(self, dictData):
        ...
        for val in dictData.values():
            if isinstance(val, dict):
                ...
                numKeys = numKeys+1
        return numKeys
    def populateParams22(self, dictData, guiData, keyStr='', xCount=0, yCount=0, heading=False, dataBlock=False, containerTabID=1):
        ...
        for key, val in dictData.items():
            ...
            if isinstance(val, dict):
                try:
                    containerTabID = self.tabNames.index(key)
                    self.populateParams22(val, guiData[key], '', self.xCount, self.yCount, False, False, containerTabID)
                except:
                    containerTabID = 1
                    self.populateParams22(val, guiData[key], keyStr+key+': ', self.xCount, self.yCount, True, True, containerTabID)
                self.dataBlock = False
                continue
            else:
                if keyStr!='' and self.heading:
                    paramLabel = ttk.Label(self.tabs[containerTabID], text=keyStr, font=('bold', '11'))
                    paramLabel.place(relx=(0.02+0.5*self.xCount), rely=(self.initialYSpacing+self.ySpacing*self.yCount))
                    self.heading = False
                    self.yCount = self.yCount+1
                if guiData[key][0]=='display-no':
                    continue
                paramLabel = ttk.Label(self.tabs[containerTabID], text=key+': ')
                paramLabel.place(relx=(0.02+0.5*self.xCount), rely=(self.initialYSpacing+self.ySpacing*self.yCount))
                if self.dataBlock == False:
                    paramLabel.config(font=('bold','11'))
                paramEntry = tk.Entry(self.tabs[containerTabID])
                paramEntry.place(relx=(0.25+0.5*self.xCount), rely=(self.initialYSpacing+self.ySpacing*self.yCount))
