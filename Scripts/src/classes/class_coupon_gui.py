...
class coupon_gui():
    def __init__(self, abqPath, srcPath):
        ...
        couponJson = open(srcPath+'/databases/database_coupon.json', 'r')
        couponDictUnicode = json.load(couponJson)
        couponJson.close()
        self.couponDatabase = ast.literal_eval(json.dumps(couponDictUnicode))
        if os.path.isdir(self.srcPath+'/classes/__pycache__'):
            shutil.rmtree(self.srcPath+'/classes/__pycache__')
        self.createGUI()
    def createGUI(self):
        windowGUI = tk.Tk()
